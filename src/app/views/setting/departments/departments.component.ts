...
import { Departamento } from 'src/app/@core/interface/system.service';
...
export class DepartmentsComponent implements OnInit {
  ...
  public xDepartamento: Departamento = {
    nombre: '',
    status: undefined,
    llave: 0
  }
  ...
  public ListarDepartamento: Departamento[] = [];

  public columns: any;
  public rows: Departamento[] = [];
  public temp: Departamento[] = [];
  public search = ''
  ...
  async ngOnInit() {
    await this.ListasDepartamento();
  }

  async ListasDepartamento() {
      ...
      (data) => {
        if (data.Cuerpo.length > 0) {
          ...
          data.Cuerpo.map((e: any) => {
            if (Array.isArray(this.ListarDepartamento)) {
              this.ListarDepartamento.push(e);
            }
          });
          this.rows = this.ListarDepartamento;
          this.temp = this.rows
        } else {
          this.loadingTable = 2
        }
  ...
  async Registrar() {
    this.xAPI.funcion = "MPPRE_C_Departamento"
    this.xAPI.parametros = ''
    this.xAPI.valores = JSON.stringify(this.xDepartamento)
    await this.apiService.Ejecutar(this.xAPI).subscribe({
      next: (data) => {
        if (data.tipo == 1) {
          this.swal.Swal1('top-end', 'success', `<h5><strong><font color="red">${this.xDepartamento.nombre}</font></strong> </br> Registrado Exitosamente!</h5>`, 3000)
          this.ListarDepartamento = []
          this.rows = []
          this.ListasDepartamento()
  ...
  ModalActualizar(modalId: string, row: any) {
    this.xDepartamento.id = row.id
    this.btnShow = true
    const modal = document.getElementById(modalId) as HTMLElement;
    this.xDepartamento = row;
    if (modal) {
      modal.style.display = 'block'; // Mostrar el modal
      modal.setAttribute('aria-hidden', 'false'); // Cambiar atributo 'aria-hidden' a 'false'
      modal.classList.add('show'); // Agregar la clase 'show' al modal
  ...
  Eliminar(row: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.xAPI.funcion = "MPPRE_D_Departamento"
  ...
  async Actualizar() {
    this.xAPI.funcion = "MPPRE_U_Departamento"
    this.xAPI.parametros = ''
    this.xAPI.valores = JSON.stringify(this.xDepartamento)
    await this.apiService.Ejecutar(this.xAPI).subscribe({
      next: (data) => {
        if (data.tipo == 1) {
          this.swal.Swal1('top-end', 'success', `<h5><strong><font color="red">${this.xDepartamento.nombre} </font></strong> </br> Actualizado Exitosamente!</h5>`, 3000)
          this.ListarDepartamento = []
          this.rows = []
          this.ListasDepartamento()
          this.LimpiarFormulario()
