...
import { Departamento, Tramites } from 'src/app/@core/interface/system.service';
...
@Component({
  ...
  styleUrl: './formalities.component.scss'
})
export class FormalitiesComponent implements OnInit {
  ...
  public ListarTramite: Tramites[] = [];

  public columns: any;
  public rows: Tramites[] = [];
  public temp: Tramites[] = [];
  public search = ''
  ...
  async ListasTramite() {
    this.xAPI.funcion = "MPPRE_R_Tramites"
    this.xAPI.parametros = ''
    this.xAPI.valores = ''
    this.apiService.Ejecutar(this.xAPI).subscribe(
      (data) => {
        if (data.Cuerpo.length > 0) {
          ...
          data.Cuerpo.map((e: any) => {
            if (Array.isArray(this.ListarTramite)) {
              this.ListarTramite.push(e);
            }
          });
          this.rows = this.ListarTramite;
          this.temp = this.rows
        } else {
          this.loadingTable = 2
        }
  ...
  async Registrar() {
    this.xAPI.funcion = "MPPRE_C_Tramites"
    this.xAPI.parametros = ''
    this.xAPI.valores = JSON.stringify(this.xTramites)
    await this.apiService.Ejecutar(this.xAPI).subscribe({
      next: (data) => {
        if (data.tipo == 1) {
          this.swal.Swal1('top-end', 'success', `<h5><strong><font color="red">${this.xTramites.nombre}</font></strong> </br> Registrado Exitosamente!</h5>`, 3000)
          this.ListarTramite = []
          this.rows = []
          this.ListasTramite()
  ...
  Eliminar(row: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.xAPI.funcion = "MPPRE_D_Tramites"
  ...
  async Actualizar() {
    this.xAPI.funcion = "MPPRE_U_Tramites"
    this.xAPI.parametros = ''
    this.xAPI.valores = JSON.stringify(this.xTramites)
    await this.apiService.Ejecutar(this.xAPI).subscribe({
      next: (data) => {
        if (data.tipo == 1) {
          this.swal.Swal1('top-end', 'success', `<h5><strong><font color="red">${this.xTramites.nombre} </font></strong> </br> Actualizado Exitosamente!</h5>`, 3000)
          this.ListarTramite = []
          this.rows = []
          this.ListasTramite()
          this.LimpiarFormulario()
