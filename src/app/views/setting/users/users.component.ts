...
export class UsersComponent implements OnInit {
  ...
  async ListaUsuarios() {
    this.xAPI.funcion = "MPPRE_R_ListaUsuarios";
      ...
      next: (data) => {
        if (data.Cuerpo.length > 0) {
          ...
          data.Cuerpo.map((e: any) => {
            if (Array.isArray(this.ListasUsuario)) {
              this.ListasUsuario.push(e);
            }
          });
          this.rows = this.ListasUsuario;
          this.temp = this.rows;
        } else {
          this.loadingTable = 2;
        }
