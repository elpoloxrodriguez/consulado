...
export class ApiService {
  ...
  ExecColeccionAuditoria(xObjeto: any, token: string): Observable<any> {
    let httpOptions = {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
   ...
   * @returns
   */
  ExecColeccion(xObjeto: any): Observable<any> {
    var url = "/v1/api/mcoleccion" + this.hash;
  ...
  GenerarCodigo(Entradas: string, funcion: string, ruta: string): string {
    ...
    json.forEach((value: { entradas: any[]; }) => {
      value.entradas.forEach((e: { alias: string; tipo: string; }) => {
